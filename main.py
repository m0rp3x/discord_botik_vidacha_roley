...
async def sos(ctx):
    await ctx.send(
        "To get your role , write !create, and then in the channel created for yourself !auth and your email\n Example: !auth test@outlook.com")


@bot.command()
async def помощь(ctx):
    await ctx.send(
        "Чтобы получить свою роль напиши !create, а после в созданном для себя канале !auth и свою почту\n Пример: !auth test@outlook.com")
